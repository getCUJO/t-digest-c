...
#define td_max_(x, y) (((x) > (y)) ? (x) : (y))
#define td_min_(x, y) (((x) < (y)) ? (x) : (y))
...
weighted_average_sorted(double x1, double w1, double x2, double w2)
{
	const double x = (x1 * w1 + x2 * w2) / (w1 + w2);
	return td_max_(x1, td_min_(x, x2));
}
...
static inline double
weighted_average(double x1, double w1, double x2, double w2)
{
	if (x1 <= x2) {
...
static unsigned int partition(double *means,
			      long long *weights,
			      unsigned int start,
			      unsigned int end,
 ...
 */
static void td_qsort(double *means,
		     long long *weights,
		     unsigned int start,
		     unsigned int end)
{
	if (start < end) {
		...
		const unsigned int pivot_idx =
			(end + start) / 2; // central pivot
...
static inline size_t cap_from_compression(double compression)
{
	if ((size_t)compression >
	    ((SIZE_MAX / sizeof(double) / TD_CAPACITY_MULTIPLIER) -
	     TD_CAPACITY_FIXEDOFFSET)) {
		return 0;
	}
	return (TD_CAPACITY_MULTIPLIER * (size_t)(compression)) +
	       TD_CAPACITY_FIXEDOFFSET;
}
				    ...
				    const double new_total_weight)
{
	...
	const double denom =
		2 * MM_PI * new_total_weight * log(new_total_weight);
	if (isinf(denom)) {
		return EDOM;
	}
...
int td_init(double compression, td_histogram_t **result)
{
	...
	histogram->nodes_weight =
		(long long *)__td_calloc(capacity, sizeof(long long));
...
double td_cdf(td_histogram_t *h, double val)
{
	...
	if (val < left_centroid_mean) {
		...
		if (width > 0) {
			...
			return (1 + (val - h->min) / width *
					    (left_centroid_weight / 2 - 1)) /
			       merged_weight_d;
		}
	...
	for (unsigned long it = 0; it < n - 1; it++) {
		...
		if (fabs(h->nodes_mean[it] - val) < PICO) {
			...
			return (weightSoFar + dw / 2) /
			       (double)h->merged_weight;
		}
		if (h->nodes_mean[it] <= val && val < h->nodes_mean[it + 1]) {
			const double node_weight = (double)h->nodes_weight[it];
			const double node_weight_next =
				(double)h->nodes_weight[it + 1];
			...
			if (node_mean_next - node_mean > 0) {
				...
				double dw =
					(node_weight + node_weight_next) / 2;

				// adjust endpoints for any singleton
				double dwNoSingleton =
					dw - leftExcludedW - rightExcludedW;

				double base = weightSoFar + node_weight / 2 +
					      leftExcludedW;
				return (base +
					dwNoSingleton * (val - node_mean) /
						(node_mean_next - node_mean)) /
				       merged_weight_d;
...
static double
td_internal_iterate_centroids_to_index(const td_histogram_t *h,
				       const double index,
				       ...
				       unsigned long *node_pos)
{
	if (left_centroid_weight > 1 && index < left_centroid_weight / 2) {
		...
		return h->min + (index - 1) / (left_centroid_weight / 2 - 1) *
					(h->nodes_mean[0] - h->min);
	}
	...
	const double right_centroid_weight =
		(double)h->nodes_weight[total_centroids - 1];
	...
	if (right_centroid_weight > 1 &&
		...
		return h->max - ((double)h->merged_weight - index - 1) /
					(right_centroid_weight / 2 - 1) *
	...
	for (; *node_pos < total_centroids - 1; (*node_pos)++) {
		...
		if (*weightSoFar + dw > index) {
			...
			return weighted_average(
				node_mean, z2, node_mean_next, z1);
		}
		*weightSoFar += dw;
	}
	...
	const double z1 =
		index - (double)h->merged_weight - right_centroid_weight / 2.0;
...
double td_quantile(td_histogram_t *h, double q)
{
	...
	unsigned long i = 0;
	return td_internal_iterate_centroids_to_index(
		h, index, left_centroid_weight, n, &weightSoFar, &i);
}
		 ...
		 size_t length)
{
	...
	if (n == 1) {
		for (size_t i = 0; i < length; i++) {
			...
			// q should be in [0,1]
			if (requested_quantile < 0.0 ||
			    requested_quantile > 1.0) {
				values[i] = (double)NAN;
			} else {
				// with one data point, all quantiles lead to Rome
	...
	for (size_t qpos = 0; qpos < length; qpos++) {
		...
		values[qpos] = td_internal_iterate_centroids_to_index(
			h,
			index,
...
static double td_internal_trimmed_mean(const td_histogram_t *h,
				       const double leftmost_weight,
				       const double rightmost_weight)
{
	...
	for (unsigned long i = 0; i < h->merged_nodes; i++) {
		...
		count_add -= td_min_(td_max_(0, leftmost_weight - count_done),
				     count_add);

		// If we have reached the upper threshold, ignore the overflowing part of the centroid.

		count_add = td_min_(td_max_(0, rightmost_weight - count_done),
				    count_add);
...
double td_trimmed_mean_symmetric(td_histogram_t *h, double proportion_to_cut)
{
	...
	if (h->merged_nodes == 0 || proportion_to_cut < 0.0 ||
	    proportion_to_cut > 1.0) {
		return (double)NAN;
	}
	...
	const double leftmost_weight =
		floor((double)h->merged_weight * proportion_to_cut);
	const double rightmost_weight =
		ceil((double)h->merged_weight * (1.0 - proportion_to_cut));
...
td_trimmed_mean(td_histogram_t *h, double leftmost_cut, double rightmost_cut)
{
	...
	if (h->merged_nodes == 0 || leftmost_cut < 0.0 || leftmost_cut > 1.0 ||
	    rightmost_cut < 0.0 || rightmost_cut > 1.0) {
		return (double)NAN;
	}
	// with one data point, all values lead to Rome
	if (h->merged_nodes == 1) {
	...
	/* translate the percentiles to counts */
	const double leftmost_weight =
		floor((double)h->merged_weight * leftmost_cut);
	const double rightmost_weight =
		ceil((double)h->merged_weight * rightmost_cut);
...
int td_add(td_histogram_t *h, double val, long long weight)
{
	...
	if (tdigest_long_long_add_safe(new_unmerged_weight, h->merged_weight) ==
	    false) {
		return EDOM;
	}
	const long long new_total_weight =
		new_unmerged_weight + h->merged_weight;
	// double-precision overflow detected
	const int overflow_res = check_td_overflow((double)new_unmerged_weight,
						   (double)new_total_weight);
	if (overflow_res != 0) {
		return overflow_res;
	}
...
int td_compress(td_histogram_t *h)
{
	...
	const double total_weight =
		(double)h->merged_weight + (double)h->unmerged_weight;
	// double-precision overflow detected
	const int overflow_res = check_td_overflow((double)h->unmerged_weight,
						   (double)total_weight);
	if (overflow_res != 0) {
		return overflow_res;
	}
	...
	for (unsigned long i = 1; i < N; i++) {
		const double proposed_weight = (double)h->nodes_weight[cur] +
					       (double)h->nodes_weight[i];
		const double z = proposed_weight * normalizer;
		// quantile up to cur
		const double q0 = weight_so_far / total_weight;
		// quantile up to cur + i
		const double q2 =
			(weight_so_far + proposed_weight) / total_weight;
		// Convert  a quantile to the k-scale
		const bool should_add = (z <= (q0 * (1 - q0))) &&
					(z <= (q2 * (1 - q2)));
		// next point will fit
		...
		if (should_add) {
			...
			const double delta =
				h->nodes_mean[i] - h->nodes_mean[cur];
			const double weighted_delta =
				(delta * (double)h->nodes_weight[i]) /
				(double)h->nodes_weight[cur];
